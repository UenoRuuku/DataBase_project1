...
(
    ...
    primary key (ta_id),
    foreign key (area_doctor) references user
        on delete set null,
    foreign key (area_nurse_master) references user
        on delete set null
);
...
(
    ...
    primary key (w_id),
    foreign key (ward_area) references treatment_area
        on delete cascade
);
...
(
    ...
    p_id int not null,
    primary key (ps_id),
    foreign key (curr_report) references NAT_report
        on delete set null,
    foreign key (p_id)
        on delete cascade
);
